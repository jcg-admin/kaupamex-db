...
def check_schemas(cfg_app: dict, log: Logger) -> bool:
    """Check 5: schemas kaupamex_core y kaupamex_core_qa existen."""
    try:
        ...
        ok = True
        for s in ['kaupamex_core', 'kaupamex_core_qa']:
            if s in schemas:
...
def check_permisos_readonly(cfg_readonly: dict, log: Logger) -> bool:
    ...
    ok = True
    cfg = {**cfg_readonly, 'database': 'kaupamex_core'}
    try:
        cnx = connect(cfg)
        ...
        try:
            query_one(cnx, "SELECT COUNT(*) FROM information_schema.tables "
                           "WHERE table_schema='kaupamex_core'")
            log.ok("readonly: SELECT en kaupamex_core OK")
        except MySQLError as e:
...
def check_permisos_app_prod(cfg_app: dict, log: Logger) -> bool:
    ...
    """
    ok = True
    cfg = {**cfg_app, 'database': 'kaupamex_core'}
    try:
        cnx = connect(cfg)
        cur = cnx.cursor()
        ...
        try:
            cur.execute("DROP DATABASE kaupamex_core")
...
def check_permisos_app_qa(cfg_app_qa: dict, log: Logger) -> bool:
    """Check 8: app puede ALL PRIVILEGES en kaupamex_core_qa."""
    ok = True
    cfg = {**cfg_app_qa, 'database': 'kaupamex_core_qa'}
    try:
        cnx = connect(cfg)
        cur = cnx.cursor()
...
def main():
    ...
    checks = [
        ...
        (4, 8, "Schemas kaupamex_core y kaupamex_core_qa existen",
