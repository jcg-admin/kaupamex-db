...
BACKUP_OUTPUT_DIR="${BACKUP_BASE}/${TIMESTAMP}"
LOG="${BACKUP_OUTPUT_DIR}/backup.log"
CHECKSUMS="${BACKUP_OUTPUT_DIR}/${TIMESTAMP}_checksums.md5"
MANIFEST="${BACKUP_OUTPUT_DIR}/${TIMESTAMP}_MANIFEST.txt"
...
for repo in "${REPOS[@]}"; do
    ...
    git -C "${ORIGEN}/${repo}" bundle create \
        "${BACKUP_OUTPUT_DIR}/${TIMESTAMP}_${repo}.bundle" \
        --all
    git bundle verify "${BACKUP_OUTPUT_DIR}/${TIMESTAMP}_${repo}.bundle" | \
...
for repo in "${REPOS[@]}"; do
    log "[tar.gz] $repo ..."
    tar -czf "${BACKUP_OUTPUT_DIR}/${TIMESTAMP}_${repo}.tar.gz" \
        -C "$ORIGEN" "$repo"
    SIZE=$(du -sh "${BACKUP_OUTPUT_DIR}/${TIMESTAMP}_${repo}.tar.gz" | cut -f1)
...
echo "" | tee -a "$LOG"
log "Artefactos generados en: $BACKUP_OUTPUT_DIR"
log "=== FIN BACKUP ==="
